...
const Lobby = (() => {
  ...
  const GAME_ICONS = { tictactoe: '✕○', connect4: '🔴', chess: '♟️', checkers: '⛀' };
  ...
  function bindEvents() {
    ...
    // Keyboard shortcut for create-room-name
  ...
  function doJoinPrivate() {
    ...
    SocketClient.joinRoom(pendingJoinId, pw);
    closeJoinModal();
  }
    ...
    filtered.forEach(room => {
      ...
      card.innerHTML = `
        ...
        <div class="room-card-meta">
          <span class="meta-tag">${GAME_ICONS[room.gameType] || '🎮'} ${room.gameLabel}</span>
          <span class="meta-tag" style="color:${LEVEL_COLORS[room.level]}">${capitalize(room.level)}</span>
          ${room.isPrivate ? '<span class="meta-tag">🔒 Private</span>' : ''}
        </div>
        <div class="room-card-footer">
          <div class="player-pips">
            ${Array.from({ length: room.maxPlayers }, (_, i) =>
              `<div class="pip ${i < room.playerCount ? 'filled' : ''}">
                ${i < room.playerCount ? '👤' : ''}
              </div>`
            ).join('')}
